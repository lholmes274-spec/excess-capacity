...
const { data: overlappingBookings } = await query;
   ...
   if (isForSale) {
    url = `/checkout?listing_id=${listing.id}&transaction_type=sale&guest=true&guest_email=${encodeURIComponent(guestEmail)}&guest_name=${encodeURIComponent(guestName)}&guest_phone=${encodeURIComponent(guestPhone)}`;
       ...
       url = `/checkout?listing_id=${listing.id}&transaction_type=booking&start_date=${startDate}&end_date=${endDate}&days=1&appointment_type=${appointmentType}&guest_email=${encodeURIComponent(guestEmail)}&guest_name=${encodeURIComponent(guestName)}&guest_phone=${encodeURIComponent(guestPhone)}`;

       if (listing.booking_mode === "time_slots") {
         url += `&time_slot=${selectedTime}`;
       }
   } else {
     url = `/checkout?listing_id=${listing.id}&transaction_type=booking&start_date=${startDate}&end_date=${endDate}&days=${Number(days || 1)}&appointment_type=${appointmentType}&guest_email=${encodeURIComponent(guestEmail)}&guest_name=${encodeURIComponent(guestName)}&guest_phone=${encodeURIComponent(guestPhone)}`;
    ...
    {listing.booking_mode === "time_slots" && (
      ...
      {listing.booking_mode === "request" && (
        <div className="mt-4 p-4 bg-yellow-50 border border-yellow-300 rounded-lg text-sm text-yellow-800">
          After checkout, the provider will contact you to coordinate and confirm a specific appointment time.
        </div>
      )}
